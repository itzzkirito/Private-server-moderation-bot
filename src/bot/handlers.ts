...
export async function handlePresenceUpdate(
  ...
  if (presence.guild.id !== cfg.guildID) {
    console.log(
      `Vanity: Presence update for different guild: ${presence.guild.id} (expected: ${cfg.guildID})`
    );
    return;
  }
  ...
  console.log(
    `Vanity: ✅ Presence update received for user ${presence.user.username} (ID: ${presence.user.id}) in guild ${presence.guild.id}`
  );

  // Log presence details for debugging
  if (presence.activities) {
    console.log(
      `Vanity: User ${presence.user.username} has ${presence.activities.length} activities`
    );
    presence.activities.forEach((act, i) => {
      console.log(
        `Vanity: Activity ${i}: Type=${act.type}, Name='${act.name}', State='${act.state}'`
      );
    });
  } else {
  ...
  try {
    ...
    console.log(
      `Vanity: Processing vanity check for user ${presence.user.username}`
    );
 ...
 */
function isRoleSafe(role: any): boolean {
 ...
 */
function getCustomStatusTextFromPresence(presence: Presence | null): string {
  if (!presence) {
  ...
  console.log(
    `Vanity: Checking ${presence.activities?.length || 0} activities for custom status`
  );
  ...
  if (presence.activities) {
    for (let i = 0; i < presence.activities.length; i++) {
      ...
      console.log(
        `Vanity: Activity ${i} - Type: ${activity.type}, Name: '${activity.name}', State: '${activity.state}', Details: '${activity.details}'`
      );
 ...
 */
async function updateVanityRoleWithPresence(
  client: Client,
  member: GuildMember,
  presence: Presence,
  bot: Bot
  ...
  if (member.guild.id !== cfg.guildID) {
    return;
  }

  // Skip cooldown if set to 0 for instant checking
  if (cfg.vanityCooldown > 0) {
  ...
  try {
    ...
    if (cfg.vanityRoleID) {
      try {
        vanityRole = await guild.roles.fetch(cfg.vanityRoleID);
      } catch (error) {
    ...
    if (!vanityRole && cfg.vanityRoleName) {
      const roles = await guild.roles.fetch();
      vanityRole =
        roles.find((role) => role.name === cfg.vanityRoleName) || null;
    }

    if (!vanityRole) {
      console.log(
        `Vanity: Role not found (ID: ${cfg.vanityRoleID}, Name: ${cfg.vanityRoleName})`
      );
      return;
    }
    ...
    if (!isRoleSafe(vanityRole)) {
      console.log(
        `Vanity: Role ${vanityRole.name} has dangerous permissions, skipping`
      );
      return;
    }
    ...
    const vanityLower = cfg.vanityString.toLowerCase();
    ...
    if (hasVanity && !hasRole) {
      ...
      try {
        ...
        console.log(
          `Vanity: ✅ Successfully added role ${vanityRole.name} to user ${member.user.username}`
        );
          ...
          try {
            ...
            if (channel && channel.isTextBased() && !channel.isDMBased()) {
              const embed = new EmbedBuilder()
                .setDescription(
                  `🩷 ${vanityRole} thanks for putting our vanity in your status, keep supporting!`
                )
        ...
        console.log(
          `Vanity: ERROR adding vanity role to ${member.user.username}: ${error}`
        );
        console.log(
          "Vanity: Check bot permissions - needs 'Manage Roles' and role must be below bot's role"
        );
      }
      ...
      console.log(
        `Vanity: Removing role ${vanityRole.name} from user ${member.user.username} (status doesn't match) in guild ${guildID}`
      );

      // Validate IDs before making API call
      if (!guildID || !member.user.id || !vanityRole.id) {
        console.log(
          `Vanity: ERROR - Invalid IDs (GuildID: ${guildID}, UserID: ${member.user.id}, RoleID: ${vanityRole.id})`
        );
        return;
      }
      ...
      try {
        ...
        console.log(
          `Vanity: Successfully removed role ${vanityRole.name} from user ${member.user.username}`
        );
      } catch (error) {
        console.log(
          `Vanity: Error removing vanity role from ${member.user.username}: ${error}`
        );
      }
    } else if (hasVanity && hasRole) {
      console.log(
        `Vanity: User ${member.user.username} already has role ${vanityRole.name}`
      );
    } else {
      console.log(
        `Vanity: User ${member.user.username} doesn't have vanity in status and doesn't have role`
      );
    }
  } catch (error) {
 ...
 */
export async function checkAllMembersForVanity(
  client: Client,
  bot: Bot
): Promise<void> {
  if (!cfg.vanityEnabled) {
  ...
  console.log(
    `Vanity: Config - Enabled: ${cfg.vanityEnabled}, String: '${cfg.vanityString}', RoleID: '${cfg.vanityRoleID}', RoleName: '${cfg.vanityRoleName}'`
  );
  ...
  try {
    ...
    if (cfg.vanityRoleID) {
      try {
        ...
        if (vanityRole) {
          console.log(
            `Vanity: Found role by ID: ${vanityRole.name} (${vanityRole.id})`
          );
        }
      } catch (error) {
        // Role not found by ID
    ...
    if (!vanityRole && cfg.vanityRoleName) {
      ...
      if (vanityRole) {
        console.log(
          `Vanity: Found role by name: ${vanityRole.name} (${vanityRole.id})`
        );
      }
    }
    ...
    for (let i = 0; i < members.length; i++) {
      ...
      if (!status) {
        ...
        if (hasRole) {
          ...
          try {
            ...
            console.log(
              `Vanity: Error removing role from ${member.user.username} (no status): ${error}`
            );
          }
      ...
      const hasVanity = status
        .toLowerCase()
        .includes(cfg.vanityString.toLowerCase());
      checked++;
      ...
      if (hasVanity && !hasRole) {
        ...
        try {
          ...
          console.log(
            `Vanity: ERROR adding role to ${member.user.username} (GuildID: ${guildID}, UserID: ${member.user.id}, RoleID: ${vanityRole.id}): ${error}`
          );
        }
        ...
        try {
          ...
          console.log(
            `Vanity: ERROR removing role from ${member.user.username} (GuildID: ${guildID}, UserID: ${member.user.id}, RoleID: ${vanityRole.id}): ${error}`
          );
        }
        // Small delay after API call
        await new Promise((resolve) => setTimeout(resolve, 50));
      ...
      if ((i + 1) % 100 === 0) {
        console.log(
          `Vanity: Progress - Checked: ${i + 1}/${members.length}, Added: ${added}, Removed: ${removed}`
        );
      }
    }

    console.log(
      `Vanity: ✅ Finished - Checked: ${checked}, Added: ${added}, Removed: ${removed}, Skipped (no status): ${skipped}`
    );
  } catch (error) {
